...
async def delete_document(
    ...
    source_id: Optional[str] = Query(
        None, description="Unique source identifier (if None, deletes entire session)"
    ),
):
    """
    ...
    if clean_source_id:
        ...
        chroma_result = delete_chunks_by_source(
            source_id=clean_source_id, session_id=clean_session_id
        )
        mongo_result = delete_file_from_session(
            session_id=clean_session_id, source_id=clean_source_id
        )
    else:
    ...
    results["success"] = chroma_result.get("success", False) and mongo_result.get(
        "success", False
    )
