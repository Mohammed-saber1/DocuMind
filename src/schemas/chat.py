...
class ChatRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None, 
        description="Session ID for conversation history tracking"
    )
    source_id: Optional[str] = Field(
        None,
        description="Source ID for filtering by a specific file"
    )
...
class ChatResponse(BaseModel):
    ...
    context_found: Optional[bool] = Field(
        None,
        description="Whether relevant context was found in the knowledgebase"
    )
