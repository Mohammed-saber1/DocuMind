...
def save_to_mongodb(json_path: str, session_id: str = None):
    ...
    try:
        ...
        print(f"📥 Successfully saved extraction to MongoDB: {result.inserted_id} (Session: None)")
...
def save_batch_to_mongodb(json_paths: list, session_id: str, author: str):
    ...
    try:
        ...
        print(f"📥 Successfully {'updated' if result.matched_count else 'created'} batch {session_id} to MongoDB")
...
def delete_file_from_session(session_id: str, source_id: str) -> dict:
    ...
    try:
        ...
        if result.modified_count == 0:
            return {"success": True, "deleted_count": 0, "message": "File not found in session"}
...
def delete_full_session(session_id: str) -> dict:
    ...
    try:
        ...
        if result.deleted_count > 0:
            ...
            return {"success": False, "error": f"Session {sess_id} not found", "deleted_count": 0}
...
def get_all_chat_sessions() -> list:
    ...
    try:
        ...
        sessions = []
        for doc in collection.find({}, {"_id": 1, "message_count": 1, "created_at": 1, "last_updated": 1}):
            sessions.append({
                "session_id": doc["_id"],
