...
class DocumentMetadata(BaseModel):
    ...
    summary: str = Field(
        description="A 2-5 sentence semantic summary of the document content"
    )
...
class TableAnalysis(BaseModel):
    ...
    sheet_purposes: Dict[str, str] = Field(
        default_factory=dict, description="Mapping of sheet names to their purpose"
    )
    insights: List[str] = Field(
        default_factory=list,
        description="Key metrics, patterns, or business implications",
    )
    ...
    column_descriptions: Optional[Dict[str, str]] = Field(
        None, description="Meaning of specific columns"
    )
    key_statistics: List[str] = Field(
        default_factory=list, description="Computed statistics or notable values"
    )
...
async def run_agent(
    base_dir, source, source_id, file_hash, author="", user_description=None
):
        ...
        with open(tables_path, "r", encoding="utf-8") as f:
            ...
            for idx, table in enumerate(tables_data[:3], 1):
                ...
                if headers:
                    ...
                    if len(data) > 0:
                        tables_info += (
                            f"Headers: {', '.join(str(h) for h in data[0])}\n"
                        )

    # Check for image analysis (OCR + VLM)
    images_analysis_path = os.path.join(
        base_dir, "images", "analysis.json"
    )  # VLM results
    ocr_analysis_path = os.path.join(
        base_dir, "images", "ocr_analysis.json"
    )  # OCR results
    images_info = ""
    ...
    has_content = (
        (len(text_for_llm.strip()) > 10) or table_count > 0 or len(all_images_data) > 0
    )

    if not has_content:
        logger.warning(
            f"⚠️ No content found for {source_id}. Skipping LLM call to prevent hallucination."
        )
        # Create a basic summary from user description or filename
        basic_summary = (
            user_description
            if user_description and len(user_description) > 5
            else f"Image file: {source_id}"
        )
    ...
    print(
        f"🤖 Calling LLM for parsing... (Tables: {table_count}, Images: {len(all_images_data)})"
    )
...
async def analyze_tables_with_llm(base_dir):
    ...
    for idx, table in enumerate(tables_data, 1):
        ...
        if headers:
            tables_text += (
                "Headers: | " + " | ".join(str(h)[:50] for h in headers) + " |\n"
            )
...
"""
    ...
    try:
        ...
        print(
            f"Response preview: {response_text[:200] if 'response_text' in locals() else 'N/A'}"
        )
        return None
