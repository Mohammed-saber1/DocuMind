...
def get_embeddings():
    ...
    return OllamaEmbeddings(
        model=settings.llm.embedding_model, base_url=settings.llm.embedding_base_url
    )


def token_splitter_chunking(
    original_text: str, chunk_size: int = 512, chunk_overlap: int = 64
) -> List[str]:
    """
...
def process_document_for_rag(
    text_content: str, method: str = "token", **kwargs
) -> List[str]:
    """
            ...
            for col_idx, header in enumerate(headers):
                if col_idx < len(row):
                    ...
                    if value and str(value).strip():
                        ...
                        meta_key = "".join(
                            c for c in meta_key if c.isalnum() or c == "_"
                        )[:50]
                        row_metadata[meta_key] = str(value)

            if row_parts:
                # Create the chunk text
                chunk_text = f"[{sheet_name} - Row {row_idx + 1}] " + ", ".join(
                    row_parts
                )
