...
This service interacts with Vision Language Models (specifically Qwen-VL-Chat) to provides
descriptions of images that could not be reliably processed by OCR.
...
Usage:
------
It is primarily called by the `document_pipeline` when `run_ocr_on_images` indicates failure or low confidence.
"""
import os
import json
import base64
...
def analyze_extracted_images(base_dir, image_paths):
    ...
    for img_path in images_to_process:
        try:
            ...
            # Call remote VLM API
            result = _call_vlm_api(img_path, settings.vlm.api_url, settings.vlm.timeout)
            ...
            if result:
                ...
                results.append({
                    ...
                    # "path": dest_path,  # Removed to avoid saving local temp paths to MongoDB
...
def _call_vlm_api(image_path: str, api_url: str, timeout: int = 60) -> Dict:
    """
    Call the remote VLM API (OpenAI-compatible) to analyze an image.
    
    Args:
        image_path: Path to the image file
        api_url: URL of the VLM API endpoint (e.g., http://host:port/v1/chat/completions)
    ...
    try:
        ...
        payload = {
            "model": "Qwen/Qwen2.5-VL-7B-Instruct",  # Or assume default
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": "Describe this image in detail. If it's a chart or graph, explain what data it shows."
                        },
                        {
                            "type": "image_url",
                            "image_url": {
...
def analyze_single_image(image_path: str) -> Dict:
    ...
    """
    settings = get_settings()
    return _call_vlm_api(image_path, settings.vlm.api_url, settings.vlm.timeout)
