...
def preprocess_excel_data(table_data):
    ...
    for row in table_data:
        ...
        for cell in row:
            if cell is None or (
                isinstance(cell, str)
                and cell.strip().lower() in ["none", "null", "nan", ""]
            ):
                cleaned_row.append("")
    ...
    if cleaned_data:
        ...
        for col_idx in range(num_cols):
            ...
            has_data = any(
                row[col_idx] != "" for row in cleaned_data if col_idx < len(row)
            )
            if has_data:
        ...
        if cols_to_keep:
            cleaned_data = [
                [row[i] for i in cols_to_keep if i < len(row)] for row in cleaned_data
            ]
